...
public extension Array {
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Array<Future<Output>>> where ParameterType == Array<Future<Input>>  {
		return fmap { $0.fmap { $0.map(transform) } }
	}
	func mapTT <Input,Output> (_ transform: (Input) -> Output) -> Array<Array<Optional<Output>>> where ParameterType == Array<Optional<Input>>  {
		return fmap { $0.fmap { $0.fmap(transform) } }
	}
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Array<Reader<Secondary3,Output>>> where ParameterType == Array<Reader<Secondary3,Input>>  {
		return fmap { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Effect<Future<Output>>> where ParameterType == Effect<Future<Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Effect<Reader<Secondary3,Output>>> where ParameterType == Effect<Reader<Secondary3,Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Future<Future<Output>>> where ParameterType == Future<Future<Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Future<Reader<Secondary3,Output>>> where ParameterType == Future<Reader<Secondary3,Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Optional<Future<Output>>> where ParameterType == Optional<Future<Input>>  {
		return fmap { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Optional<Reader<Secondary3,Output>>> where ParameterType == Optional<Reader<Secondary3,Input>>  {
		return fmap { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Reader<Secondary2,Future<Output>>> where ParameterType == Reader<Secondary2,Future<Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Reader<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Reader<Secondary2,Reader<Secondary3,Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Result<Future<Output>,Secondary2>> where ParameterType == Result<Future<Input>,Secondary2>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Result<Reader<Secondary3,Output>,Secondary2>> where ParameterType == Result<Reader<Secondary3,Input>,Secondary2>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<State<Secondary2,Future<Output>>> where ParameterType == State<Secondary2,Future<Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<State<Secondary2,Reader<Secondary3,Output>>> where ParameterType == State<Secondary2,Reader<Secondary3,Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Writer<Secondary2,Future<Output>>> where ParameterType == Writer<Secondary2,Future<Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Array<Writer<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Writer<Secondary2,Reader<Secondary3,Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
...
public extension Effect {
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Array<Future<Output>>> where ParameterType == Array<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Array<Reader<Secondary3,Output>>> where ParameterType == Array<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Effect<Future<Output>>> where ParameterType == Effect<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Effect<Reader<Secondary3,Output>>> where ParameterType == Effect<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Future<Future<Output>>> where ParameterType == Future<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Future<Reader<Secondary3,Output>>> where ParameterType == Future<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Optional<Future<Output>>> where ParameterType == Optional<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Optional<Reader<Secondary3,Output>>> where ParameterType == Optional<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Reader<Secondary2,Future<Output>>> where ParameterType == Reader<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Reader<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Reader<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Result<Future<Output>,Secondary2>> where ParameterType == Result<Future<Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Result<Reader<Secondary3,Output>,Secondary2>> where ParameterType == Result<Reader<Secondary3,Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<State<Secondary2,Future<Output>>> where ParameterType == State<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<State<Secondary2,Reader<Secondary3,Output>>> where ParameterType == State<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Writer<Secondary2,Future<Output>>> where ParameterType == Writer<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Effect<Writer<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Writer<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
...
public extension Future {
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Array<Future<Output>>> where ParameterType == Array<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Array<Reader<Secondary3,Output>>> where ParameterType == Array<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Effect<Future<Output>>> where ParameterType == Effect<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Effect<Reader<Secondary3,Output>>> where ParameterType == Effect<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Future<Future<Output>>> where ParameterType == Future<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Future<Reader<Secondary3,Output>>> where ParameterType == Future<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Optional<Future<Output>>> where ParameterType == Optional<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Optional<Reader<Secondary3,Output>>> where ParameterType == Optional<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Reader<Secondary2,Future<Output>>> where ParameterType == Reader<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Reader<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Reader<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Result<Future<Output>,Secondary2>> where ParameterType == Result<Future<Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Result<Reader<Secondary3,Output>,Secondary2>> where ParameterType == Result<Reader<Secondary3,Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<State<Secondary2,Future<Output>>> where ParameterType == State<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<State<Secondary2,Reader<Secondary3,Output>>> where ParameterType == State<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Writer<Secondary2,Future<Output>>> where ParameterType == Writer<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Future<Writer<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Writer<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
...
public extension Optional {
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Array<Future<Output>>> where ParameterType == Array<Future<Input>>  {
		return fmap { $0.fmap { $0.map(transform) } }
	}
	func mapTT <Input,Output> (_ transform: (Input) -> Output) -> Optional<Array<Optional<Output>>> where ParameterType == Array<Optional<Input>>  {
		return fmap { $0.fmap { $0.fmap(transform) } }
	}
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Array<Reader<Secondary3,Output>>> where ParameterType == Array<Reader<Secondary3,Input>>  {
		return fmap { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Effect<Future<Output>>> where ParameterType == Effect<Future<Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Effect<Reader<Secondary3,Output>>> where ParameterType == Effect<Reader<Secondary3,Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Future<Future<Output>>> where ParameterType == Future<Future<Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Future<Reader<Secondary3,Output>>> where ParameterType == Future<Reader<Secondary3,Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Optional<Future<Output>>> where ParameterType == Optional<Future<Input>>  {
		return fmap { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Optional<Reader<Secondary3,Output>>> where ParameterType == Optional<Reader<Secondary3,Input>>  {
		return fmap { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Reader<Secondary2,Future<Output>>> where ParameterType == Reader<Secondary2,Future<Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Reader<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Reader<Secondary2,Reader<Secondary3,Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Result<Future<Output>,Secondary2>> where ParameterType == Result<Future<Input>,Secondary2>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Result<Reader<Secondary3,Output>,Secondary2>> where ParameterType == Result<Reader<Secondary3,Input>,Secondary2>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<State<Secondary2,Future<Output>>> where ParameterType == State<Secondary2,Future<Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<State<Secondary2,Reader<Secondary3,Output>>> where ParameterType == State<Secondary2,Reader<Secondary3,Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Writer<Secondary2,Future<Output>>> where ParameterType == Writer<Secondary2,Future<Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Optional<Writer<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Writer<Secondary2,Reader<Secondary3,Input>>  {
		return fmap { $0.map { $0.map(transform) } }
	}
...
public extension Reader {
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Array<Future<Output>>> where ParameterType == Array<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Array<Reader<Secondary3,Output>>> where ParameterType == Array<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Effect<Future<Output>>> where ParameterType == Effect<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Effect<Reader<Secondary3,Output>>> where ParameterType == Effect<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Future<Future<Output>>> where ParameterType == Future<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Future<Reader<Secondary3,Output>>> where ParameterType == Future<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Optional<Future<Output>>> where ParameterType == Optional<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Optional<Reader<Secondary3,Output>>> where ParameterType == Optional<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Reader<Secondary2,Future<Output>>> where ParameterType == Reader<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Reader<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Reader<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Result<Future<Output>,Secondary2>> where ParameterType == Result<Future<Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Result<Reader<Secondary3,Output>,Secondary2>> where ParameterType == Result<Reader<Secondary3,Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,State<Secondary2,Future<Output>>> where ParameterType == State<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,State<Secondary2,Reader<Secondary3,Output>>> where ParameterType == State<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Writer<Secondary2,Future<Output>>> where ParameterType == Writer<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Reader<Environment,Writer<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Writer<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
...
public extension Result {
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Array<Future<Output>>,Failure> where ParameterType == Array<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Array<Reader<Secondary3,Output>>,Failure> where ParameterType == Array<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Effect<Future<Output>>,Failure> where ParameterType == Effect<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Effect<Reader<Secondary3,Output>>,Failure> where ParameterType == Effect<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Future<Future<Output>>,Failure> where ParameterType == Future<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Future<Reader<Secondary3,Output>>,Failure> where ParameterType == Future<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Optional<Future<Output>>,Failure> where ParameterType == Optional<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Optional<Reader<Secondary3,Output>>,Failure> where ParameterType == Optional<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Reader<Secondary2,Future<Output>>,Failure> where ParameterType == Reader<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Reader<Secondary2,Reader<Secondary3,Output>>,Failure> where ParameterType == Reader<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Result<Future<Output>,Secondary2>,Failure> where ParameterType == Result<Future<Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Result<Reader<Secondary3,Output>,Secondary2>,Failure> where ParameterType == Result<Reader<Secondary3,Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<State<Secondary2,Future<Output>>,Failure> where ParameterType == State<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<State<Secondary2,Reader<Secondary3,Output>>,Failure> where ParameterType == State<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Writer<Secondary2,Future<Output>>,Failure> where ParameterType == Writer<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Result<Writer<Secondary2,Reader<Secondary3,Output>>,Failure> where ParameterType == Writer<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
...
public extension State {
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Array<Future<Output>>> where ParameterType == Array<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Array<Reader<Secondary3,Output>>> where ParameterType == Array<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Effect<Future<Output>>> where ParameterType == Effect<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Effect<Reader<Secondary3,Output>>> where ParameterType == Effect<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Future<Future<Output>>> where ParameterType == Future<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Future<Reader<Secondary3,Output>>> where ParameterType == Future<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Optional<Future<Output>>> where ParameterType == Optional<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Optional<Reader<Secondary3,Output>>> where ParameterType == Optional<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Reader<Secondary2,Future<Output>>> where ParameterType == Reader<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Reader<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Reader<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Result<Future<Output>,Secondary2>> where ParameterType == Result<Future<Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Result<Reader<Secondary3,Output>,Secondary2>> where ParameterType == Result<Reader<Secondary3,Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,State<Secondary2,Future<Output>>> where ParameterType == State<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,State<Secondary2,Reader<Secondary3,Output>>> where ParameterType == State<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Writer<Secondary2,Future<Output>>> where ParameterType == Writer<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> State<Model,Writer<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Writer<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
...
public extension Writer {
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Array<Future<Output>>> where ParameterType == Array<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Array<Reader<Secondary3,Output>>> where ParameterType == Array<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Effect<Future<Output>>> where ParameterType == Effect<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Effect<Reader<Secondary3,Output>>> where ParameterType == Effect<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Future<Future<Output>>> where ParameterType == Future<Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Future<Reader<Secondary3,Output>>> where ParameterType == Future<Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Optional<Future<Output>>> where ParameterType == Optional<Future<Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Optional<Reader<Secondary3,Output>>> where ParameterType == Optional<Reader<Secondary3,Input>>  {
		return map { $0.fmap { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Reader<Secondary2,Future<Output>>> where ParameterType == Reader<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Reader<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Reader<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Result<Future<Output>,Secondary2>> where ParameterType == Result<Future<Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Result<Reader<Secondary3,Output>,Secondary2>> where ParameterType == Result<Reader<Secondary3,Input>,Secondary2>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,State<Secondary2,Future<Output>>> where ParameterType == State<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,State<Secondary2,Reader<Secondary3,Output>>> where ParameterType == State<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Writer<Secondary2,Future<Output>>> where ParameterType == Writer<Secondary2,Future<Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
	...
	func mapTT <Secondary3,Secondary2,Input,Output> (_ transform: @Sendable @escaping (Input) -> Output) -> Writer<Log,Writer<Secondary2,Reader<Secondary3,Output>>> where ParameterType == Writer<Secondary2,Reader<Secondary3,Input>>  {
		return map { $0.map { $0.map(transform) } }
	}
