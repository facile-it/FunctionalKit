...
extension ArrayType where ParameterType: ArrayType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Array<Effect<Output>>> {
...
extension ArrayType where ParameterType: EffectType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Effect<Array<Output>>> {
        return fmap { $0.map { $0.fmap(transform) } }
    }
}

extension ArrayType where ParameterType: EffectType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Effect<Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}

extension ArrayType where ParameterType: EffectType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Effect<Future<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}

extension ArrayType where ParameterType: EffectType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Effect<Optional<Output>>> {
        return fmap { $0.map { $0.fmap(transform) } }
    }
}

extension ArrayType where ParameterType: EffectType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Effect<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}

extension ArrayType where ParameterType: EffectType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Effect<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}

extension ArrayType where ParameterType: EffectType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Effect<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}

extension ArrayType where ParameterType: EffectType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Effect<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension ArrayType where ParameterType: FutureType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Future<Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension ArrayType where ParameterType: OptionalType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Optional<Effect<Output>>> {
        return fmap { $0.fmap { $0.map(transform) } }
    }
}
...
extension ArrayType where ParameterType: ReaderType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Reader<ParameterType.EnvironmentType,Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension ArrayType where ParameterType: ResultType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Result<ParameterType.ErrorType,Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension ArrayType where ParameterType: StateType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<State<ParameterType.StateParameterType,Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension ArrayType where ParameterType: WriterType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Array<Writer<ParameterType.LogType,Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension EffectType where ParameterType: ArrayType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Array<Array<Output>>> {
        return map { $0.fmap { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: ArrayType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Array<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ArrayType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Array<Future<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ArrayType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Array<Optional<Output>>> {
        return map { $0.fmap { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: ArrayType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Array<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ArrayType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Array<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ArrayType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Array<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ArrayType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Array<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: EffectType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Effect<Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: EffectType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Effect<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: EffectType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Effect<Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: EffectType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Effect<Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: EffectType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Effect<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: EffectType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Effect<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: EffectType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Effect<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: EffectType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Effect<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: FutureType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Future<Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: FutureType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Future<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: FutureType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Future<Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: FutureType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Future<Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: FutureType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Future<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: FutureType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Future<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: FutureType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Future<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: FutureType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Future<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: OptionalType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Optional<Array<Output>>> {
        return map { $0.fmap { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: OptionalType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Optional<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: OptionalType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Optional<Future<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: OptionalType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Optional<Optional<Output>>> {
        return map { $0.fmap { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: OptionalType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Optional<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: OptionalType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Optional<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: OptionalType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Optional<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: OptionalType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Optional<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ReaderType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Reader<ParameterType.EnvironmentType,Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: ReaderType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Reader<ParameterType.EnvironmentType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ReaderType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Reader<ParameterType.EnvironmentType,Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ReaderType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Reader<ParameterType.EnvironmentType,Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: ReaderType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Reader<ParameterType.EnvironmentType,Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ReaderType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Reader<ParameterType.EnvironmentType,Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ReaderType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Reader<ParameterType.EnvironmentType,State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ReaderType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Reader<ParameterType.EnvironmentType,Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ResultType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Result<ParameterType.ErrorType,Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: ResultType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Result<ParameterType.ErrorType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ResultType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Result<ParameterType.ErrorType,Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ResultType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Result<ParameterType.ErrorType,Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: ResultType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Result<ParameterType.ErrorType,Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ResultType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Result<ParameterType.ErrorType,Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ResultType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Result<ParameterType.ErrorType,State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: ResultType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Result<ParameterType.ErrorType,Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: StateType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<State<ParameterType.StateParameterType,Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: StateType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<State<ParameterType.StateParameterType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: StateType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<State<ParameterType.StateParameterType,Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: StateType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<State<ParameterType.StateParameterType,Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: StateType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<State<ParameterType.StateParameterType,Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: StateType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<State<ParameterType.StateParameterType,Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: StateType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<State<ParameterType.StateParameterType,State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: StateType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<State<ParameterType.StateParameterType,Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: WriterType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Writer<ParameterType.LogType,Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: WriterType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Writer<ParameterType.LogType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: WriterType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Writer<ParameterType.LogType,Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: WriterType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Writer<ParameterType.LogType,Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension EffectType where ParameterType: WriterType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Writer<ParameterType.LogType,Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: WriterType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Writer<ParameterType.LogType,Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: WriterType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Writer<ParameterType.LogType,State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension EffectType where ParameterType: WriterType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Effect<Writer<ParameterType.LogType,Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension FutureType where ParameterType: ArrayType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Array<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}
...
extension FutureType where ParameterType: EffectType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Effect<Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension FutureType where ParameterType: EffectType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Effect<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension FutureType where ParameterType: EffectType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Effect<Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension FutureType where ParameterType: EffectType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Effect<Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension FutureType where ParameterType: EffectType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Effect<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension FutureType where ParameterType: EffectType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Effect<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension FutureType where ParameterType: EffectType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Effect<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension FutureType where ParameterType: EffectType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Effect<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension FutureType where ParameterType: FutureType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Future<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension FutureType where ParameterType: OptionalType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Optional<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}
...
extension FutureType where ParameterType: ReaderType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Reader<ParameterType.EnvironmentType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension FutureType where ParameterType: ResultType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Result<ParameterType.ErrorType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension FutureType where ParameterType: StateType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<State<ParameterType.StateParameterType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension FutureType where ParameterType: WriterType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Future<Writer<ParameterType.LogType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension OptionalType where ParameterType: ArrayType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Array<Effect<Output>>> {
        return fmap { $0.fmap { $0.map(transform) } }
    }
}
...
extension OptionalType where ParameterType: EffectType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Effect<Array<Output>>> {
        return fmap { $0.map { $0.fmap(transform) } }
    }
}

extension OptionalType where ParameterType: EffectType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Effect<Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}

extension OptionalType where ParameterType: EffectType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Effect<Future<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}

extension OptionalType where ParameterType: EffectType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Effect<Optional<Output>>> {
        return fmap { $0.map { $0.fmap(transform) } }
    }
}

extension OptionalType where ParameterType: EffectType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Effect<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}

extension OptionalType where ParameterType: EffectType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Effect<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}

extension OptionalType where ParameterType: EffectType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Effect<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}

extension OptionalType where ParameterType: EffectType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Effect<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension OptionalType where ParameterType: FutureType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Future<Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension OptionalType where ParameterType: OptionalType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Optional<Effect<Output>>> {
        return fmap { $0.fmap { $0.map(transform) } }
    }
}
...
extension OptionalType where ParameterType: ReaderType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Reader<ParameterType.EnvironmentType,Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension OptionalType where ParameterType: ResultType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Result<ParameterType.ErrorType,Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension OptionalType where ParameterType: StateType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<State<ParameterType.StateParameterType,Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension OptionalType where ParameterType: WriterType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Optional<Writer<ParameterType.LogType,Effect<Output>>> {
        return fmap { $0.map { $0.map(transform) } }
    }
}
...
extension ReaderType where ParameterType: ArrayType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Array<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}
...
extension ReaderType where ParameterType: EffectType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Effect<Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension ReaderType where ParameterType: EffectType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Effect<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension ReaderType where ParameterType: EffectType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Effect<Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension ReaderType where ParameterType: EffectType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Effect<Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension ReaderType where ParameterType: EffectType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Effect<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension ReaderType where ParameterType: EffectType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Effect<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension ReaderType where ParameterType: EffectType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Effect<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension ReaderType where ParameterType: EffectType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Effect<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ReaderType where ParameterType: FutureType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Future<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ReaderType where ParameterType: OptionalType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Optional<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}
...
extension ReaderType where ParameterType: ReaderType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Reader<ParameterType.EnvironmentType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ReaderType where ParameterType: ResultType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Result<ParameterType.ErrorType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ReaderType where ParameterType: StateType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,State<ParameterType.StateParameterType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ReaderType where ParameterType: WriterType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Reader<EnvironmentType,Writer<ParameterType.LogType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ResultType where ParameterType: ArrayType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Array<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}
...
extension ResultType where ParameterType: EffectType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Effect<Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension ResultType where ParameterType: EffectType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Effect<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension ResultType where ParameterType: EffectType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Effect<Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension ResultType where ParameterType: EffectType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Effect<Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension ResultType where ParameterType: EffectType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Effect<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension ResultType where ParameterType: EffectType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Effect<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension ResultType where ParameterType: EffectType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Effect<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension ResultType where ParameterType: EffectType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Effect<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ResultType where ParameterType: FutureType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Future<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ResultType where ParameterType: OptionalType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Optional<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}
...
extension ResultType where ParameterType: ReaderType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Reader<ParameterType.EnvironmentType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ResultType where ParameterType: ResultType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Result<ParameterType.ErrorType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ResultType where ParameterType: StateType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,State<ParameterType.StateParameterType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension ResultType where ParameterType: WriterType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Result<ErrorType,Writer<ParameterType.LogType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension StateType where ParameterType: ArrayType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Array<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}
...
extension StateType where ParameterType: EffectType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Effect<Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension StateType where ParameterType: EffectType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Effect<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension StateType where ParameterType: EffectType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Effect<Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension StateType where ParameterType: EffectType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Effect<Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension StateType where ParameterType: EffectType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Effect<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension StateType where ParameterType: EffectType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Effect<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension StateType where ParameterType: EffectType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Effect<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension StateType where ParameterType: EffectType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Effect<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension StateType where ParameterType: FutureType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Future<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension StateType where ParameterType: OptionalType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Optional<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}
...
extension StateType where ParameterType: ReaderType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Reader<ParameterType.EnvironmentType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension StateType where ParameterType: ResultType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Result<ParameterType.ErrorType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension StateType where ParameterType: StateType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,State<ParameterType.StateParameterType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension StateType where ParameterType: WriterType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> State<StateParameterType,Writer<ParameterType.LogType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension WriterType where ParameterType: ArrayType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Array<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}
...
extension WriterType where ParameterType: EffectType, ParameterType.ParameterType: ArrayType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Effect<Array<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension WriterType where ParameterType: EffectType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Effect<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension WriterType where ParameterType: EffectType, ParameterType.ParameterType: FutureType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Effect<Future<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension WriterType where ParameterType: EffectType, ParameterType.ParameterType: OptionalType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Effect<Optional<Output>>> {
        return map { $0.map { $0.fmap(transform) } }
    }
}

extension WriterType where ParameterType: EffectType, ParameterType.ParameterType: ReaderType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Effect<Reader<ParameterType.ParameterType.EnvironmentType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension WriterType where ParameterType: EffectType, ParameterType.ParameterType: ResultType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Effect<Result<ParameterType.ParameterType.ErrorType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension WriterType where ParameterType: EffectType, ParameterType.ParameterType: StateType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Effect<State<ParameterType.ParameterType.StateParameterType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}

extension WriterType where ParameterType: EffectType, ParameterType.ParameterType: WriterType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Effect<Writer<ParameterType.ParameterType.LogType,Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension WriterType where ParameterType: FutureType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Future<Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension WriterType where ParameterType: OptionalType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Optional<Effect<Output>>> {
        return map { $0.fmap { $0.map(transform) } }
    }
}
...
extension WriterType where ParameterType: ReaderType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Reader<ParameterType.EnvironmentType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension WriterType where ParameterType: ResultType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Result<ParameterType.ErrorType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension WriterType where ParameterType: StateType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,State<ParameterType.StateParameterType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
...
extension WriterType where ParameterType: WriterType, ParameterType.ParameterType: EffectType {
    public func mapTT <Output> (_ transform: @escaping (ParameterType.ParameterType.ParameterType) -> Output) -> Writer<LogType,Writer<ParameterType.LogType,Effect<Output>>> {
        return map { $0.map { $0.map(transform) } }
    }
}
